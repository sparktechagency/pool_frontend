...
export const updatePassApi = async (
  data: { current: string; password: string; password_confirmation: string },
  token: string
) => {
  return await howl({ link: "/update-password", method: "post", data, token });
};

export const editAccApi = async (
  data: {
    full_name: string;
    email: string;
    bio: string;
...
export const sendMessage = async (
  data: { receiver_id: string | number; message: string },
  token: string
) => {
